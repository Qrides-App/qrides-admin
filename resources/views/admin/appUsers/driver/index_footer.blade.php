...
<script>
        ...
        $(document).ready(function () {
            ...
            // Special flow for host status (driver application decision) with admin note and explicit approve/reject
            $body.addEventListener('change', async e => {
                if (!e.target.matches('.hoststatusdata')) return;
                const checkbox = e.target;
                const id = checkbox.dataset.id;
                const desiredStatus = checkbox.checked ? 1 : 0; // 1=approved, 0=rejected/pending

                const { value: formValues } = await Swal.fire({
                    title: desiredStatus ? 'Approve driver?' : 'Reject driver?',
                    html:
                        '<textarea id=\"swal-note\" class=\"swal2-textarea\" placeholder=\"Optional note to driver\"></textarea>',
                    showCancelButton: true,
                    confirmButtonText: desiredStatus ? 'Approve & notify' : 'Reject & notify',
                ...
                if (!formValues) {
                    checkbox.checked = !desiredStatus; // revert
                    return;
                }
                ...
                try {
                    await $.ajax({
                        ...
                        data: { status: desiredStatus, pid: id, note: formValues.note, _token: $csrfToken }
                    });
