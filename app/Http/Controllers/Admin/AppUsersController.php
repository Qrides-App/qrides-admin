...
class AppUsersController extends Controller
{
    ...
    /**
     * Normalized list of driver documents we expect.
     */
    private function driverDocumentKeys(): array
    {
        return [
        ...
        if ($statusData) {
            ...
            if ($note !== '') {
                AppUserMeta::updateOrCreate(
                    ['user_id' => $user->id, 'meta_key' => 'host_decision_note'],
                    ['meta_value' => $note]
                );
            }
            ...
            if ($user->host_status) {
                ...
                $valuesArray['decision_note'] = $note;
                $this->sendAllNotifications($valuesArray, $request->pid, $template_id);
            } else {
                // fallback custom notification for rejection/pending
                $decisionLabel = ((string) $request->status === '0') ? 'rejected' : 'pending';
                $subject = 'Driver application '.$decisionLabel;
                $body = 'Hello '.$user->first_name.', your driver application has been '.$decisionLabel.'.';
                if ($note !== '') {
                    $body .= ' Note: '.$note;
                }
                $phone = $user->phone_country.$user->phone;
                try {
                    $this->sendSMS($subject, $body, $phone);
                } catch (\Throwable $e) {
                    // ignore SMS errors
                }
                try {
    ...
    public function updateIdentifyGrpc(Request $request)
    {
        ...
        if ($verified) {
            ...
            if (! $firestoreDocId || ! $firestoreDocExists) {
                ...
                $firestoreDocId = $this->resolveFirestoreDocId($firestoreDoc);
                $user->update(['firestore_id' => $firestoreDocId]);
                $user['firestore_id'] = $firestoreDocId;
            }

            $user->update(['host_status' => (int) $request->verified === 1 ? 1 : 0]);
    ...
    private function resolveFirestoreDocId($firestoreDoc): ?string
    {
        if (is_string($firestoreDoc)) {
            return basename($firestoreDoc);
        }

        if (is_object($firestoreDoc) && method_exists($firestoreDoc, 'id')) {
