...
class PayoutMethodController extends Controller
{
    ...
    {
        if (! $this->hasPermission('payout_method_edit', 'payout_edit')) {
            return response()->json([
                'status' => 403,
    ...
    {
        abort_if(! $this->hasPermission('payout_method_delete', 'payout_delete'), Response::HTTP_FORBIDDEN, '403 Forbidden');
    ...
    private function hasPermission(string $preferred, ?string $fallback = null): bool
    {
        if (Gate::allows($preferred)) {
            return true;
        }

        return $fallback ? Gate::allows($fallback) : false;
    }
}
